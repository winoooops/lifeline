...
**The harness must run on a feature branch, never on `main`.** The interactive main agent works on a feature branch in the **primary checkout** — it does **not** enter a worktree for itself. The harness Python orchestrator runs in that same primary checkout on the same feature branch.

> Subagents and harness Coder subprocesses may use worktrees as a future optimization (per-feature isolation under `.claude/worktrees/<feature>/`), but the **current** harness architecture does not yet implement per-feature worktrees; all Coder iterations happen in the same working tree as the orchestrator.
...
**Do NOT call `EnterWorktree`** — it puts the main agent into a worktree the user's dev server and diff viewer can't see. If your project also installs a `block-main-commit.sh` PreToolUse hook, it will reject commits on `main` regardless, so the primary-checkout feature branch is the correct and safe location.

### Why This Matters

- The user typically runs the project's dev server (e.g. `npm run dev`, `cargo run`, `python manage.py runserver`) from the primary checkout. Edits in `.claude/worktrees/` are invisible to that view. If the main agent works in a worktree, the user can't verify progress live.
- Running on `main` would violate branch protection — most projects block commits there.
- A feature branch in the primary checkout is cheaply cleaned up once merged.
...
# Stay here — run /lifeline:github-review for code review cycles
...
1. **Feature branch** — `git branch --show-current` must NOT return `main`. From the primary checkout, `git checkout -b <branch-name>`. Do NOT `EnterWorktree` for the main agent.
...
`npm run review:fix` or the `/lifeline:github-review` skill in Claude Code provides a self-driving fix loop: fetch Codex review → fix findings → push → poll for next review → repeat until clean (max 10 rounds).
...
The harness skills are distributed as the `lifeline` Claude Code plugin. If `/lifeline:loop`, `/lifeline:review`, and `/lifeline:github-review` are not available, install the plugin:

```bash
# 1. Add the project's local marketplace (one-time)
/plugin marketplace add .

# 2. Install the lifeline plugin
/plugin install lifeline@lifeline
```

The marketplace definition is at `.claude-plugin/marketplace.json` (project root) and the skill sources are at `skills/`.
