...
Allowlist-based validation for bash commands run by harness coder/reviewer
sessions. Project-agnostic baseline — extend per-project via
`harness/.policy_allow.local` (one base command per line).
"""

import re
import shlex

# Baseline commands allowed for general development. Project-specific tools
# (e.g. cargo, mvn, gradle, poetry, dotnet, ruby) should be added either here
# or via the user-local `.policy_allow.local` file.
ALLOWED_COMMANDS = {
    # File inspection
