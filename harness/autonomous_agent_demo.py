...
autonomously build the project rooted at ``--project-dir`` from an
``app_spec.md`` description. Project- and language-agnostic.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Autonomous Development Harness",
    )
    ...
    parser.add_argument(
        ...
        help="Project directory (default: parent of harness/)",
    )
    ...
    *,
    ...
    if not stdin_isatty:
        print(
            ...
            "or run /lifeline:loop's Phase 3 step manually after "
...
def clean_runtime_files(project_dir: Path) -> None:
    ...
    default placeholder template), silently replacing the user's real
    spec. If a user genuinely wants to wipe their spec they can delete
    it manually; ``--clean`` only touches machine-generated artifacts.
    """
