...
If the objective references paths outside the current git repository (e.g. `/tmp/...`, `/etc/...`, files in a sibling directory), the three evidence blocks above will be empty. Inspect those paths directly using read-only tools (`cat`, `ls`, `head`). If a path comes from `files_touched`, use the line exactly as shown; paired mode already rejected hints containing HTML-special path characters. Your sandbox is `read-only` — this is permitted and expected.
