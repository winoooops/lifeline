...
# /lifeline:upsource-review — Fix PR Review Findings (Connector-Aware Self-Driving Loop)
...
- **Per-cycle artifacts:** under `.lifeline-upsource-review/` (gitignored). See `references/cleanup-recovery.md`.
...
SKILL_DIR="skills/upsource-review"
[ -d "$SKILL_DIR" ] || SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null)/skills/upsource-review"
...
body to `.lifeline-upsource-review/cycle-${ROUND}-loud-fail-<source>.txt` and
`exit 1`.
...
```bash
ensure_excluded ".lifeline-upsource-review/"   # self-bootstrap consumer repo's .git/info/exclude
mkdir -p .lifeline-upsource-review
DIFF_PATCH=".lifeline-upsource-review/cycle-${ROUND}-diff.patch"
PROMPT_FILE=".lifeline-upsource-review/cycle-${ROUND}-verify-prompt.md"
RESULT_JSON=".lifeline-upsource-review/cycle-${ROUND}-verify-result.json"
EVENTS_LOG=".lifeline-upsource-review/cycle-${ROUND}-verify-events.log"
STDERR_LOG=".lifeline-upsource-review/cycle-${ROUND}-verify-stderr.log"
...
In all three cases artifacts under `.lifeline-upsource-review/` are
...
2. **"Codex verify aborts with `verify_timeout`."** External `timeout 300`
   ...
   `.lifeline-upsource-review/cycle-N-aborted/incident.md` records the
...
3. **"Codex verify aborts with `verify_error`."** Non-zero exit from
   ...
   `.lifeline-upsource-review/cycle-N-verify-stderr.log` for the exact
   error string.
...
7. **"Plugin cache out of sync with working tree."** After editing
   ...
   `cp -r skills/upsource-review ~/.claude/plugins/cache/lifeline/lifeline/0.0.1/skills/`.
...
Per-cycle artifacts under `.lifeline-upsource-review/` follow a strict
