...
#   [ -d "$SKILL_DIR" ] || SKILL_DIR="$(git rev-parse --show-toplevel)/skills/upsource-review"
...
# ensure_excluded
# ----------------------------------------------------------------------------
# Idempotently add a pattern to .git/info/exclude so the consumer repo's
# git status stays clean when the skill writes scratch dirs. Per-repo (not
# committed), so installing lifeline never pollutes the user's .gitignore.
#
# Args:
#   $1  pattern  — gitignore pattern (e.g. ".lifeline-upsource-review/")
#
# No-op outside a git repo or when the exclude file is unreachable.
ensure_excluded() {
  local pattern="$1"
  local excludes
...
# loop_start_scan
# ----------------------------------------------------------------------------
# Scan `.lifeline-upsource-review/` for prior-cycle artifacts before a new run
...
loop_start_scan() {
  if [ ! -d .lifeline-upsource-review ]; then return 0; fi

  local aborted_dirs
  aborted_dirs=$(find .lifeline-upsource-review -maxdepth 1 -type d -name 'cycle-*-aborted' 2>/dev/null)
  ...
  if [ -n "$aborted_dirs" ]; then
    ...
    echo "Once resolved, remove them with: rm -rf .lifeline-upsource-review/cycle-*-aborted/" >&2
    echo "Then re-run /lifeline:upsource-review." >&2
    exit 1
  fi

  # Wipe orphan non-aborted artifacts.
  local orphans
  orphans=$(find .lifeline-upsource-review -maxdepth 1 -type f -name 'cycle-*' 2>/dev/null | wc -l)
  if [ "$orphans" -gt 0 ]; then
    find .lifeline-upsource-review -maxdepth 1 -type f -name 'cycle-*' -delete
...
cleanup_on_clean_exit() {
  ...
  while IFS= read -r f; do
    rm -f "$f"
    count=$((count + 1))
  done < <(find .lifeline-upsource-review -maxdepth 1 -type f -name 'cycle-*')
