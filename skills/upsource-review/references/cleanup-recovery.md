...
`.lifeline-upsource-review/` (gitignored). This file documents:
...
The skill writes `cycle-${ROUND}-*` files to `.lifeline-upsource-review/`
...
| Round N aborts → loop exits                                                                         | **Preserve everything** in `.lifeline-upsource-review/`. Print recovery instructions (below).                                                                                                                                                                                                                   |
...
2. **Loop state lives elsewhere.** Persistent state is GitHub +
   ...
   `.lifeline-upsource-review/cycle-*-aborted/`. Stash would be a third
   surface.
...
```bash
rm -rf .lifeline-upsource-review/
```
...
⚠️ Loop exited at round $ROUND because $REASON.
  ...
  Last verify result: .lifeline-upsource-review/cycle-${ROUND}-verify-result.json
