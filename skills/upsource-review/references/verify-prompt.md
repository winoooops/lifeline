...
The directory is excluded per-repo via `.git/info/exclude` (bootstrapped
by `ensure_excluded` in `helpers.sh`). Step 6's commit explicitly enumerates files
...
```bash
ABORT_DIR=".lifeline-upsource-review/cycle-${ROUND}-aborted"
mkdir -p "$ABORT_DIR"
